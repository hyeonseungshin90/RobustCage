#pragma once

#include <cfloat>
#include <queue>
#include "Dense"

#include "Config.hh"
#include "CageSimplifier/Topo/TopoOperations.h"
#include "CageSimplifier/Topo/EdgeCollapser.h"
#include "CageSimplifier/Geom/GeometryCheck.h"

namespace Cage
{
namespace CageSimp
{

class CollapseStage
{
  friend struct CollapseStageTestAccess;
public:
  // input
  ParamCollapseStage* param;
  SMeshT* om;
  SMeshT* rm;
  DFaceTree* ot;
  LightDFaceTree* lrt;
  FaceGrid* og;

  double original_diagonal_length;
  size_t candidate_points_size;
  bool allow_negtive;
  double max_distance_error;
  // See EdgeCollapser::set_rail_support.
  bool use_rail_support = false;
public:
  CollapseStage(
    SMeshT* original, SMeshT* cage, ParamCollapseStage* p,
    DFaceTree* original_tree, LightDFaceTree* remeshing_tree,
    FaceGrid* original_grid,
    double _original_diagonal_length);
  ~CollapseStage();

  void do_collapse(size_t edge_num_to_collapse, size_t& total_collapsed_edge_num);
  // Replaces the legacy (fast) Phase 3 by repeatedly collapsing edges until the target
  // vertex count is reached. Each collapse chooses its new vertex position
  // before the topology change is committed.
  // A zero target removes the vertex/pass limits and stops after two rebuilt
  // passes without a collapse, using the initial mean edge length as its scale.
  // Repeated calls rebuild candidates after flip/relocation while retaining
  // the global target edge length for the same requested vertex count.
  // Returns the number of accepted collapses in this call.
  size_t do_phase3_energy_simplification(size_t target_vertices_num);
  void update(size_t _candidate_points_size, bool _allow_negtive, double _max_distance_error);
private:
  double avg_edge_length;
  double avg_source_edge_length;
  bool avg_source_edge_length_initialized;
  double phase3_target_edge_length;
  bool phase3_target_edge_length_initialized;
  size_t phase3_target_vertices_num;
  std::vector<size_t> update_states;

  struct CollapseEdgeReward
  {
    EdgeHandle eh;
    size_t state;
    double reward;
    Vec3d new_point;

    CollapseEdgeReward() = default;
    CollapseEdgeReward(EdgeHandle _eh, size_t _state, double _reward, const Vec3d& _new_point) :
      eh(_eh), state(_state), reward(_reward), new_point(_new_point)
    {}

    bool operator<(const CollapseEdgeReward& rhs)const { return reward < rhs.reward; }
  };
  typedef std::priority_queue<CollapseEdgeReward> CollapseEdgeRewardQueue;
  CollapseEdgeRewardQueue edges_to_collapse;

  struct Phase3EdgeReward
  {
    EdgeHandle eh;
    size_t state;
    double reward;
    Vec3d initial_point;

    Phase3EdgeReward() = default;
    Phase3EdgeReward(EdgeHandle _eh, size_t _state, double _reward, const Vec3d& _initial_point) :
      eh(_eh), state(_state), reward(_reward), initial_point(_initial_point)
    {}

    bool operator<(const Phase3EdgeReward& rhs)const
    {
      return reward < rhs.reward;
    }
  };
  typedef std::priority_queue<Phase3EdgeReward> Phase3EdgeRewardQueue;
  Phase3EdgeRewardQueue phase3_edges_to_collapse;
  OpenMesh::VPropHandleT<Eigen::Matrix4d> phase3_qem_quadric_prop;
  bool phase3_qem_quadric_prop_added = false;

  enum class EdgeSide
  {
    Inside,
    Outside,
    OnSurface,
    Unknown
  };

  struct EdgeSideStats
  {
    size_t inside = 0;
    size_t outside = 0;
    size_t on_surface = 0;
    size_t unknown = 0;

    size_t total() const { return inside + outside + on_surface + unknown; }
  };

  struct PlaneConstraint
  {
    Vec3d normal;
    double offset = 0.0;
    double weight = 1.0;
  };

  struct CollapseFanEdge
  {
    Vec3d from;
    Vec3d to;
    Vec3d apex;
  };

  // Local geometric data for evaluating one edge-collapse placement x.
  // This object does not run Newton; it is shared by the quadric proxy solve,
  // fallback candidate checks, and optional Newton refinement.
  struct Phase3PlacementContext
  {
    EdgeHandle edge;
    std::vector<HalfedgeHandle> halfedges;
    std::vector<CollapseFanEdge> fan_edges;
    std::vector<Vec3d> neighbor_points;
    std::vector<PlaneConstraint> plane_constraints;
    std::set<int> ignored_faces;
    Eigen::Matrix4d qem_matrix = Eigen::Matrix4d::Zero();
    bool use_qem_matrix = false;
    Vec3d endpoint0;
    Vec3d endpoint1;
    Vec3d tangential_smoothing_point;
    Vec3d midpoint;
    double local_scale = 1.0;
    double edge_curvature = 0.0;
    double avg_cage_edge_length = 1.0;
  };

  struct NewtonDerivatives
  {
    double energy = DBL_MAX;
    Vec3d gradient;
    double hessian[3][3] = {};
  };

  struct Phase3PlacementDecision
  {
    Vec3d point;
    double priority_energy = DBL_MAX;
    double min_quality = 1.0;
    double nonlinear_residual = 0.0;
    bool attempted_newton = false;
    bool accepted_newton = false;
    bool newton_failed = false;
    bool used_fallback = false;
  };

  struct Phase3QueueComponents
  {
    double plane = 0.0;
    double triangle_quality = 0.0;
    double uniformity = 0.0;
  };

  Phase3QueueComponents phase3_queue_component_maxima;

  std::vector<Vec3d> generate_candidate_points_for_collapse(EdgeHandle e, EdgeCollapser& edge_collapser);
  bool find_collapse_hausdorff_deviation(
    EdgeHandle eh, double& local_hd_before, double& local_hd_after, Vec3d& new_point);
  bool refine_collapse_placement_with_newton(
    EdgeHandle eh, EdgeCollapser& edge_collapser, Vec3d& new_point, double& energy,
    const Vec3d* initial_point = nullptr);
  void initialize_collapse_edges_reward();
  void update_after_collapsing(VertexHandle collapsed_center);
  bool is_optimization_collapse_placement_method() const;
  bool is_phase3_quadric_strategy() const;
  bool is_phase3_linear_solve_strategy() const;
  bool is_phase3_qem_original_strategy() const;
  bool is_phase3_newton_solve_strategy() const;
  bool is_trust_region_solver_mode() const;
  bool is_exact_reject_robustness_mode() const;
  bool is_exact_backtracking_robustness_mode() const;
  Phase3PlacementContext make_phase3_placement_context(EdgeHandle eh, EdgeCollapser& edge_collapser) const;
  NewtonDerivatives finite_difference_newton_derivatives(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  NewtonDerivatives approximate_newton_derivatives(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  NewtonDerivatives autodiff_newton_derivatives(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_newton_energy(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_plane_energy(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_plane_only_energy(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double calc_triangle_quality(SMeshT* mesh, FaceHandle fh) const;
  double evaluate_triangle_quality_energy(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  Eigen::Matrix4d build_phase3_triangle_quality_surrogate_quadric(
    const Phase3PlacementContext& ctx) const;
  double evaluate_phase3_triangle_quality_surrogate(
    const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_pre_collapse_triangle_quality_penalty(const Phase3PlacementContext& ctx) const;
  Eigen::Matrix4d build_phase3_uniformity_difference_quadric(
    const Phase3PlacementContext& ctx) const;
  double evaluate_phase3_uniformity_difference(
    const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_uniformity_energy(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_phase3_proxy_energy(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  Phase3QueueComponents evaluate_phase3_queue_components(
    const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double evaluate_phase3_queue_score(const Phase3QueueComponents& components) const;
  double evaluate_phase3_refinement_residual(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double calc_phase3_fan_min_quality(const Phase3PlacementContext& ctx, const Vec3d& x) const;
  double source_uniformity_target_length(const Phase3PlacementContext& ctx, const Vec3d& sample_point) const;
  double source_edge_length_at(const Vec3d& sample_point) const;
  bool collapse_target_valid(EdgeCollapser& edge_collapser, const Vec3d& x) const;
  bool phase3_placement_satisfies_hard_constraints(
    const Phase3PlacementContext& ctx, EdgeCollapser& edge_collapser, const Vec3d& x) const;
  void set_phase3_edge_collapser_flags(EdgeCollapser& edge_collapser) const;
  bool choose_phase3_collapse_placement(
    EdgeHandle eh, EdgeCollapser& edge_collapser, const Vec3d& queued_point,
    Phase3PlacementDecision& decision, double& newton_seconds);
  bool solve_phase3_quadric_placement(
    const Phase3PlacementContext& ctx, Vec3d& new_point, double& energy) const;
  bool select_phase3_linear_solve_line_search_candidate(
    const Phase3PlacementContext& ctx, EdgeCollapser& edge_collapser,
    const Vec3d& quadric_point, Vec3d& selected_point, double& selected_energy) const;
  void initialize_phase3_qem_quadrics();
  void update_phase3_qem_quadric_after_collapse(VertexHandle center_vh, const Eigen::Matrix4d& quadric);
  Eigen::Matrix4d phase3_qem_quadric(VertexHandle vh) const;
  bool select_phase3_feasibility_fallback(
    EdgeHandle eh, const Phase3PlacementContext& ctx, EdgeCollapser& edge_collapser,
    Vec3d& new_point, double& energy, bool plane_only_score = false) const;
  bool compute_phase3_queue_placement_candidate(EdgeHandle eh, Vec3d& new_point, double& energy);
  bool compute_phase3_queue_candidate_data(
    EdgeHandle eh, Vec3d& new_point, Phase3QueueComponents& components);
  bool enqueue_phase3_candidate(EdgeHandle eh, size_t state);
  void refresh_phase3_average_lengths();
  void initialize_phase3_target_edge_length(size_t target_vertices_num);
  void initialize_phase3_candidates();
  void update_phase3_after_collapsing(VertexHandle collapsed_center);
  EdgeSide classify_edge_side(EdgeHandle eh) const;
  EdgeSideStats collect_candidate_edge_side_stats() const;
  void add_edge_side(EdgeSideStats& stats, EdgeSide side) const;
  void log_edge_side_stats(const char* label, const EdgeSideStats& stats) const;


  inline EdgeCollapser new_edge_collapser()
  {
    EdgeCollapser collapser(om, rm, ot, lrt, og);
    collapser.set_rail_support(use_rail_support);
    return collapser;
  }
};

}// namespace CageSimp
}// namespace Cage
