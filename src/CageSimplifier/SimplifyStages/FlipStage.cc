...
size_t FlipStage::do_quality_flip()
{
  ...
  const auto evaluate_candidate = [&](EdgeHandle eh, double& reward)
  {
    ...
    return std::isfinite(reward) && reward > quality_epsilon;
  };
  const auto enqueue = [&](EdgeHandle eh)
  {
    ...
    // Test quality before any intersection queries.
    double reward = 0.0;
    if (evaluate_candidate(eh, reward))
  ...
  while (!edges_to_flip.empty())
  {
    ...
    // triangle quality before using the priority.
    double reward = 0.0;
    if (!evaluate_candidate(eh, reward))
      continue;
